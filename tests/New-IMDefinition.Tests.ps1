...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
  Context "Validate parameters" {
    $defaultParamCount = 13 # SupportsShouldProcess
    [object[]]$params = (Get-ChildItem "function:\$CommandName").Parameters.Keys
    $knownParameters = @('Name', 'InstallManager', 'RequiredVersion', 'AutoUpgrade', 'AutoRemove', 'Parameter', 'ExemptMachine', 'Repository', 'Scope')
