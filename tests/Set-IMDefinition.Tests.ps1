...
Describe "$CommandName Unit Tests" -Tag 'UnitTests' {
  Context "Validate parameters" {
    $defaultParamCount = 13 # supports ShouldProcess
    [object[]]$params = (Get-ChildItem "function:\$CommandName").Parameters.Keys
    $knownParameters = @('Name', 'InstallManager', 'IMDefinition', 'RequiredVersion', 'AutoUpgrade', 'AutoRemove', 'Parameter', 'ExemptMachine', 'Repository', 'Scope')
